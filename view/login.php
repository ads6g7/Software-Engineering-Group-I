...
<?php
	...
	if ($rows == 1) {
		...
		if ($rows == 1) 
		{
			...
			if ($rows==1)
			{
				header('Location: https://groupi-softwareeng.rhcloud.com/professordashboard.html');
			}
			
			//Check if user is admin, redirect
			else if (strcmp($username, "admin")==0)
			{
				header('Location: https://groupi-softwareeng.rhcloud.com/admindashboard.html');
			}
			
			//otherwise redirect to applicantdash
			else
			{
				header('Location: https://groupi-softwareeng.rhcloud.com/applicantdashboard.html');
			}
		}
	}
?>
